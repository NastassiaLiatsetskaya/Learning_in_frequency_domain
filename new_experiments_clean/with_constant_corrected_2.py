...
from mogptk.gpr.singleoutput import  ExponentialKernel,RationalQuadraticKernel,PolynomialKernel, ConstantKernel, SquaredExponentialKernel,  MaternKernel
from mogptk.gpr.multioutput import LinearModelOfCoregionalizationKernel, IndependentMultiOutputKernel
...
def func2(t):
    result= (torch.tanh(((func1(t)) / 100) * 2) ).clone().detach()
    return result
...
kernel6=MaternKernel(input_dims=12)
kernel7=ConstantKernel(input_dims=12)
kernels=AddKernel([ kernel3, kernel5, kernel6,kernel7 ])
kernels2=kernels.clone()
kernel_first=IndependentMultiOutputKernel([kernels,kernels2],output_dims= 2)
kernel_sec=kernel_first.clone().requires_grad_(True)
kernel_third=kernel_first.clone().requires_grad_(True)
kernel_fourth=kernel_first.clone().requires_grad_(True)
kernel_fifth=kernel_first.clone().requires_grad_(True)
kernel_sixth=kernel_first.clone().requires_grad_(True)
...
model1=mogptk.Model(data1,kernel_first )
...
model1.save("LMC1_10_const6")
model2.save("LMC2_10_const6")
model3.save("LMC3_10_const6")
model4.save("LMC4_10_const6")
model5.save("LMC5_10_const6")
model6.save("LMC6_10_const6")
